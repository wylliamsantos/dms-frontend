...
async function uploadViaPresigned(input: UploadDocumentInput, onProgress: (value: number) => void) {
  ...
  const finalizePayload: FinalizeUploadPayload = {
...
function uploadToStorageWithProgress({ url, file, contentType, onProgress }: UploadToStorageParams) {
...
async function uploadViaMultipart(input: UploadDocumentInput) {
  const { file, category, metadata, isFinal, issuingDate, author, comment, filename } = input;
  ...
  if (issuingDate) formData.append('issuingDate', issuingDate);
...
export function useUploadDocument() {
  ...
  const mutation = useMutation<DocumentId, Error, UploadDocumentInput>({
