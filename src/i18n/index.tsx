...
const translate = (language: SupportedLanguage, key: string, options?: Record<string, unknown>): string => {
  const segments = key.split('.');
  let current: unknown = resources[language]?.translation;
  ...
  return current.replace(/{{(.*?)}}/g, (_: string, group: string) => {
...
export function I18nProvider({ children }: { children: ReactNode }) {
  const [language, setLanguage] = useState<SupportedLanguage>(() => getStoredLanguage() ?? 'pt');
...
export function useTranslation() {
  ...
  const t = useCallback((key: string, options?: Record<string, unknown>) => translate(language, key, options), [language]);
