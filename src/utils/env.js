...
export const env = {
    ...
    idpRedirectUri: requireEnv('VITE_IDP_REDIRECT_URI', fallbackRedirectUri ?? runtimeDefaultRedirect),
    idpPostLogoutRedirectUri: requireEnv('VITE_IDP_POST_LOGOUT_REDIRECT_URI', fallbackRedirectUri ?? runtimeDefaultRedirect),
    idpScopes: optionalEnv('VITE_IDP_SCOPES') ?? 'openid profile email',
    idpRefreshLeewaySeconds: Number(optionalEnv('VITE_IDP_REFRESH_LEEWAY_SECONDS') ?? '60'),
    idpAutoLogin: (optionalEnv('VITE_IDP_AUTO_LOGIN') ?? 'true').toLowerCase() === 'true'
};
